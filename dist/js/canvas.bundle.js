...
function init() {
  ...
  for (var i = 0; i < particleCount; i++) {
    var x = canvas.width / 2 + i * Math.cos(1);
    var y = canvas.height / 2 + i * Math.sin(1);
    particles.push(new Particle(x, y, 5, "hsl(".concat(hueIncrement * i, ", 50%, 50%)"), i));
  }
} // Animation Loop
...
function randomHue() {
  return "hsl(".concat(Math.random(), ", 50%, 50%)");
}
